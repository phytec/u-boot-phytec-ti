...
void do_board_detect(void)
{
	...
	/* Read I2C EEPROM and cache in SRAM for U-Boot proper */
	rc = phytec_eeprom_data_setup(&data, 0, EEPROM_ADDR);
	if (rc)
...
#else
	/*
	 * DM I2C is unavailable in pre-relocation (initf_dm only probes
	 * pre-reloc devices). Use the EEPROM data cached in SRAM by SPL.
	 */
	struct phytec_eeprom_data data = EEPROM_DATA;
	char *opt = phytec_get_am57_opt(&data);
